...
class ProfileController extends Controller
{
    ...
    {
        $title = '12 Zodiac - Hồ sơ của bạn';

        $users = Admin::select('id', 'name', 'email', 'phone', 'password', 'avatar', 'status', 'role')
            ->paginate(5);

        return view('admin.profile.index', ['users' => $users, 'title' => $title]);
    }

    public function edit($id, Request $request) {
        $modelUser = new Admin();
        $user = $modelUser->loadOne($id);
        $this->v['title'] = '12 Zodiac - Sửa hồ sơ';
        $this->v['user'] = $user;
    ...
    public function update($id, Request $request) {
        ...
        if ($res == null) {
            ...
            return redirect()->route('route_BackEnd_Admin_Dashboard');
        } else {
