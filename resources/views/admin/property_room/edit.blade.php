        ...
        <section class="container-fluid">
            <form action="{{ route('route_BackEnd_PropertyRoom_update', $property_rooms->id) }}" method="post" class="row">
                ...
                <div class="col-0">
                    ...
                    <div class="">
                        <input type="text" class="form-control" placeholder="Nhập tên..." value="{{ $cateRoom->name }}"
                            readonly>
                    </div>
                </div>
                <label for="date" class="col-5 col-form-label">Tên thuộc tính</label>
                <div class="col-0 form-check" style="text-align: center">
                    <div class="">
                        <ul class="ks-cboxtags">
                            @foreach ($listProperties as $index => $property)
                                <li><input type="checkbox" id="{{ $property->id }}" name="properties_id[]"
                                        value="{{ $property->id }}"
                                        {{ !empty(in_array($property->id, $idNotChecked)) ? 'checked' : '' }}><label
                                        for="{{ $property->id }}">{{ $property->name }}</label></li>
                            @endforeach
                        </ul>
                    </div>
                </div>
